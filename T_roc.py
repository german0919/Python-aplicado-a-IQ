...
#T[=]°C ; P[=]torr
	...
	def Margules(Componentes,xi):
		...
		if Componentes == ["CLOROFORMO","ETANOL"]:
			A12=0.59
			A21=1.42						
		gamma1=np.exp((A12 + 2*(A21 - A12)*xi)*(1-xi)**2)
		gamma2=np.exp((A21 + 2*(A12 - A21)*(1-xi))*xi**2)
		gammas=[gamma1,gamma2]
		return gammas
	def Antoine(Componentes,T):	
		T=T-273.15 #Esta funcion trabaja con °C
		Dic_ctes=TRO.Dic_Ant
	...
	def PHI2V(Componentes,T,P,xi):
		R=83.14
		Bij=TRO.C2VIRIAL(Componentes,T)
		Pi_s=TRO.Antoine(Componentes,T)
		d12=2*Bij[1]-Bij[2]-Bij[0]
		phi_1=np.exp((Bij[0]*(P-Pi_s[0])+P*(1-xi)**2*d12)/(R*T))
		phi_2=np.exp((Bij[2]*(P-Pi_s[1])+P*xi**2*d12)/(R*T))
	...
	def f_tr(Componentes,P,y1):	
		gammai=np.asarray([1,1])
		phi_i=np.asarray([1,1])
		Ti_sat=TRO.Tsat(Componentes,P)
		T0=y1*Ti_sat[0]+(1-y1)*Ti_sat[1]
		Pi_s=TRO.Antoine(Componentes,T0)
		Pj_s=P*(((y1*phi_i[0]/gammai[0])*(Pi_s[1]/Pi_s[0]))+(((1-y1)*phi_i[1]/gammai[1])*(Pi_s[1]/Pi_s[1])))##
		Ti=TRO.Tsat(Componentes,Pj_s)[1]		
		Pi_sat=TRO.Antoine(Componentes,Ti)
		phi_i=TRO.PHI2V(Componentes,Ti,P,y1)
		xi=np.asarray([(y1*phi_i[0]*P)/(gammai[0]*Pi_sat[0]),((1-y1)*phi_i[1]*P)/(gammai[1]*Pi_sat[1])])
		gamma_i=TRO.Margules(Componentes,xi[0])
		Pj_si=P*(((y1*phi_i[0]/gammai[0])*(Pi_s[1]/Pi_s[0]))+(((1-y1)*phi_i[1]/gammai[1])*(Pi_s[1]/Pi_s[1])))##
		Tin=TRO.Tsat(Componentes,Pj_si)[1]	#Dado a que elegimos i como el segundo componente aca se calcula Ti	
		tol=1e-5
		er=tol+1
		x_suma=0.60
		n=0
		while (er>=tol): 
			Psat=TRO.Antoine(Componentes,Tin)
			phi=TRO.PHI2V(Componentes,Tin,P,y1)
			xi=[(y1*phi[0]*P)/(gamma_i[0]*Psat[0]),((1-y1)*phi[1]*P)/(gamma_i[1]*Psat[1])]
			while (x_suma!=1): 
				xiN=[xi[0]/(xi[0]+(1-xi[0])),xi[1]/(xi[1]+(1-xi[1]))]
				xi=xiN
				xs=xi[0]+xi[1]
				x_suma=xs
				n=n+1
				break	
			while (er>=tol):
				gammas=TRO.Margules(Componentes,xi[0])	
				gamma_i=gammas
				er=np.sqrt(((gammas[0]-gamma_i[0])**2))	
				n=n+1
			Pjsat=P*(((y1*phi[0]/gammas[0])*(Psat[1]/Psat[0]))+(((1-y1)*phi[1]/gammas[1])*(Psat[1]/Psat[1])))##
			Pj_si=Pjsat
			T=TRO.Tsat(Componentes,Pjsat)[1]		
			Tin=T
			er=np.sqrt(((T-Tin)**2))	
			n=n+1
		...
		return T,xi
	#Gráfico de EVL	
	def G_tr(Componentes,P):
		y1=np.array([0.001, 0.003410345, 0.00782069 , 0.08231034, 0.14641379,
       ...
       0.86258621, 0.89668966, 0.9307931 , 0.96489655, 0.999])		
		...
		T=[TRO.f_tr(Componentes,P,y1[k])[0] for k in list(range(0,len(y1)))]
		x1=[TRO.f_tr(Componentes,P,y1[k])[1][0] for k in list(range(0,len(y1)))]
		...
		plt.grid(True)
		#Tabla
		Datos=np.transpose([y1,x1,T])
		Encabezados=["y1","x1","T_r(K)"]
		print(tabulate(Datos,headers=Encabezados,tablefmt="fancy_grid",stralign="center"))		
		return plt.show()				

#print(TRO.f_tr(["ACETONA","METANOL"],120,0.1))		
#print(TRO.G_tr(["ACETONA","METANOL"],120))
