...
#T[=]°C ; P[=]torr
	...
	def PHI2V(Componentes,T,P,xi):
		R=83.14
		Bij=PRO.C2VIRIAL(Componentes,T)
		Pi_s=PRO.Antoine(Componentes,T)
		...
		return phi_ij
	def f_pr(Componentes,T,y1):
		Pi_sat=PRO.Antoine(Componentes,T)
		gammai=np.asarray([1,1])
		phi_i=np.asarray([1,1])
		Pi=1/((y1*phi_i[0]/(gammai[0]*Pi_sat[0]))+((1-y1)*phi_i[1]/(gammai[1]*Pi_sat[1])))
		xi=np.asarray([(y1*phi_i[0]*Pi)/(gammai[0]*Pi_sat[0]),((1-y1)*phi_i[1]*Pi)/(gammai[1]*Pi_sat[1])])
		gamma_i=PRO.Margules(Componentes,xi[0])
		tol=1e-5
		er=tol+1
		x_suma=0.60
		n=0
		while (er>=tol): 
			phi=PRO.PHI2V(Componentes,T,Pi,xi[0])
			phi_i=phi
			xi=[(y1*phi_i[0]*Pi)/(gamma_i[0]*Pi_sat[0]),((1-y1)*phi_i[1]*Pi)/(gamma_i[1]*Pi_sat[1])]
			while (x_suma!=1): 
				...
				break	
			while (er>=tol):
				gammas=PRO.Margules(Componentes,xi[0])	
				gamma_i=gammas
				er=np.sqrt(((gammas[0]-gamma_i[0])**2))	
				n=n+1
			P=1/((y1*phi_i[0]/(gammas[0]*Pi_sat[0]))+((1-y1)*phi_i[1]/(gammas[1]*Pi_sat[1])))
			Pi=P					
			er=np.sqrt(((P-Pi)**2))	
			n=n+1
			#print("Se llega a la convergencia en la iteración número:",n)
		return np.asarray(P),xi,gamma_i
	#Gráfico de EVL	
	def G_pr(Componentes,T):
       ...
       0.86258621, 0.89668966, 0.9307931 , 0.96489655, 0.999])		
		...
		P=[PRO.f_pr(Componentes,T,y1[k])[0] for k in list(range(0,len(y1)))]
		x1=[PRO.f_pr(Componentes,T,y1[k])[1][0] for k in list(range(0,len(y1)))]
		...
		plt.grid(True)
		#Tabla
		Datos=np.transpose([y1,x1,P])
		Encabezados=["y1","x1","P_b(kPa)"]
		print(tabulate(Datos,headers=Encabezados,tablefmt="fancy_grid",stralign="center"))		
		return plt.show()		

#print(PRO.f_pr(["CLOROFORMO","ETANOL"],55+273.15,0.35))		
#print(PRO.G_pr(["CLOROFORMO","ETANOL"],55+273.15))		
#print(PRO.G_pr(["ACETONA","METANOL"],85+273.15))		
